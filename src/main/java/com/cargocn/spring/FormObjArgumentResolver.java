...
public class FormObjArgumentResolver implements HandlerMethodArgumentResolver {
	public FormObjArgumentResolver() { 
	}
	...
	public Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer,
		...
		Object obj = (mavContainer.containsAttribute(alias)) ? mavContainer.getModel().get(alias)
				: createAttribute(alias, parameter, binderFactory, webRequest);
	...
	private Object createAttribute(String alias, MethodParameter parameter, WebDataBinderFactory binderFactory,
	...
	private void bindParameters(NativeWebRequest request, WebDataBinder binder, String alias) {
		...
		while (enu.hasMoreElements()) {
			...
			if (paramName.startsWith(alias)) {
				newRequest.setParameter(paramName.substring(alias.length() + 1), request.getParameterValues(paramName));
			}
		}
